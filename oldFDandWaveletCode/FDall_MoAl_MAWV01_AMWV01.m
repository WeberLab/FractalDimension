...
elseif automation=='y'      %% MAW_Jan2011
    doFD='n';
    %FDdir=? take input from automation function doSlice=? doComponents=?
    %Za and Zb bins=? Bbold=input('BOLD DATA (e.g. data+orig): ','s');
    %maskfile=?
else
...
if doFD=='n',
    ...
    % This is where a section to write AFNI HEAD/BRIK files needs to be
    % added To do this you will need to make use of AFNI matlab function
    % 'Writebrik'
    
    % 1)  You will need: 'Info' made by AFNI plugin 'brikload' (top of this
    % pgm) 2)  The creation of BRIK/HEAD files of the following wlll be
    % required:
    %     bold_mean bold_std FDmapRDslow FDmapRDfast FDmapPS
    % 3)  These are all 3D variables AP,LR,SI- or 64x64 x 3 slices 4) There
    % is no need to make BRIK/HEAD for interpolated versions of these as
    % this can be done using the AFNI GUI.
...
if ROIyes=='y',
    ...
    elseif ROItype=='m'
        ...
        BW=double(MASK); % % assuming mask is single slice
